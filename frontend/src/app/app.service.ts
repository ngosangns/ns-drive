...
import * as WailsAppService from "../../bindings/ns-drive/appservice.js";
...
import {
  ...
  isValidSyncStatus,
  isValidSyncAction,
} from "./models/sync-status.interface.js";
...
export class AppService implements OnDestroy {
  ...
  async pull(profile: models.Profile) {
    ...
    this.currentId$.next(await WailsAppService.Sync(Action.Pull, profile));
  ...
  async pullWithTab(profile: models.Profile, tabId: string) {
    ...
    const taskId = await WailsAppService.SyncWithTab(
      Action.Pull,
      profile,
      tabId
    );
  ...
  async push(profile: models.Profile) {
    ...
    this.currentId$.next(await WailsAppService.Sync(Action.Push, profile));
  ...
  async bi(profile: models.Profile, resync = false) {
    ...
    this.replaceData("Bi...");
    this.currentId$.next(
      await WailsAppService.Sync(resync ? Action.BiResync : Action.Bi, profile)
    );
  ...
  stopCommand() {
    ...
    WailsAppService.StopCommand(this.currentId$.value);
  }

  stopCommandForTab(tabId: string) {
    const tab = this.tabService.getTab(tabId);
    if (!tab || !tab.currentAction || !tab.currentTaskId) return;
    WailsAppService.StopCommand(tab.currentTaskId);
  }

  async getConfigInfo() {
    console.log("AppService getConfigInfo called");
    try {
      const configInfo = await WailsAppService.GetConfigInfo();
      console.log(
  ...
  async getRemotes(): Promise<void> {
    try {
      const remotes = await WailsAppService.GetRemotes();
  ...
  async addRemote(objData: Record<string, string>): Promise<void> {
    ...
    try {
      const err = await WailsAppService.AddRemote(
        objData["name"],
        objData["type"],
        {}
      );
  ...
  async stopAddingRemote(): Promise<void> {
    try {
      // Note: StopAddingRemote is not available in the new AppService bindings
      // This functionality may need to be implemented differently
      throw new Error("StopAddingRemote not implemented in v3");
    } catch (error) {
  ...
  async deleteRemote(name: string): Promise<void> {
    ...
    try {
      await WailsAppService.DeleteRemote(name);
      await this.getRemotes();
    } catch (error) {
  ...
  async saveConfigInfo() {
    try {
      const err = await WailsAppService.UpdateProfiles(
        this.configInfo$.value.profiles
      );
      if (err) {
        throw err.message;
      }
  ...
  async exportProfiles(): Promise<void> {
    try {
      // TODO: Implement export functionality in v3
      throw new Error("Export profiles not yet implemented in v3");
    } catch (error) {
  ...
  async importProfiles(): Promise<void> {
    try {
      // TODO: Implement import functionality in v3
      throw new Error("Import profiles not yet implemented in v3");
    } catch (error) {
  ...
  async exportRemotes(): Promise<void> {
    try {
      // TODO: Implement export functionality in v3
      throw new Error("Export remotes not yet implemented in v3");
    } catch (error) {
  ...
  async importRemotes(): Promise<void> {
    try {
      // TODO: Implement import functionality in v3
      throw new Error("Import remotes not yet implemented in v3");
    } catch (error) {
